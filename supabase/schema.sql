...
-- 6. World Store / Marketplace ----------------------------------------------
-- Publishable, playable worlds. `world_config` mirrors WorldConfig in the store
-- so a listing loads straight into a fresh game. Public SELECT; creator-only writes.
create table if not exists public.worlds (
  ...
  world_config jsonb not null,                  -- WorldConfig the game loads to play
...
-- Atomic play-counter bump (avoids a read-modify-write race).
