...
const QUEST_STATUS_STYLE: Record<Quest["status"], { icon: typeof CheckCircle2; label: string; cls: string }> = {
  active:    { icon: Clock,         label: "In Progress", cls: "text-amber-400 border-amber-700/40 bg-amber-950/30" },
  completed: { icon: CheckCircle2,  label: "Completed",   cls: "text-emerald-400 border-emerald-700/40 bg-emerald-950/20" },
  failed:    { icon: XCircle,       label: "Failed",      cls: "text-red-400/70 border-red-800/40 bg-red-950/20" },
};
      ...
      <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto bg-stone-950/95 border border-amber-900/30 rounded-2xl shadow-2xl p-6 space-y-6 animate-modal-pop">
        <div className="flex items-center justify-between border-b border-amber-900/20 pb-3">
          <h2 className="flex items-center gap-2 text-lg font-bold bg-gradient-to-r from-amber-200 via-amber-400 to-orange-300 bg-clip-text text-transparent tracking-widest">
            <ScrollText size={18} className="text-amber-400" /> Traveler&apos;s Journal
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        {currentObjective && (
          <div className="space-y-2">
            <h3 className="text-xs font-bold text-amber-400/60 uppercase tracking-widest">Current Objective</h3>
        ...
        {questLog.length > 0 && (
          <div className="space-y-3">
            <h3 className="text-xs font-bold text-amber-400/60 uppercase tracking-widest">Quests</h3>
            ...
            {doneQuests.length > 0 && (
              <div className="space-y-1.5">
                <p className="text-[10px] text-stone-600 uppercase tracking-widest">Done / Failed</p>
                {doneQuests.map((q) => {
                  const s = QUEST_STATUS_STYLE[q.status];
                  const Icon = s.icon;
                  return (
        ...
        {/* Story summary */}
        <div className="space-y-2">
          <h3 className="text-xs font-bold text-amber-400/60 uppercase tracking-widest">Story Summary</h3>
          <p className="text-sm text-amber-50/80 leading-relaxed whitespace-pre-wrap">
            {storySummary || "No story recorded yet..."}
          </p>
        </div>
        ...
        {visitedLocations.length > 0 && (
          <div className="space-y-2">
            <h3 className="text-xs font-bold text-amber-400/60 uppercase tracking-widest">Visited Locations</h3>
            <div className="space-y-1.5">
        ...
        {/* World info */}
        <div className="space-y-2">
          <h3 className="text-xs font-bold text-amber-400/60 uppercase tracking-widest">World &amp; Character Info</h3>
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              <span className="text-amber-400/60">Language: </span>
              <span className="text-amber-50/80">{worldConfig?.language}</span>
            </div>
            <div>
              <span className="text-amber-400/60">Tone: </span>
              <span className="text-amber-50/80">{worldConfig?.tone}</span>
            </div>
            <div className="col-span-2">
              <span className="text-amber-400/60">Genre: </span>
              <span className="text-amber-50/80">{worldConfig?.genre}</span>
            </div>
          </div>
          {worldConfig?.character && (
            <div className="pt-2">
              <span className="text-amber-400/60 text-sm">Character: </span>
          ...
          {worldConfig?.customWorld && (
            <div className="pt-2">
              <span className="text-amber-400/60 text-sm">World Setting: </span>
          ...
          {worldConfig?.openingSeed && (
            <div className="pt-2">
              <span className="text-amber-400/60 text-sm">Opening Scene: </span>
